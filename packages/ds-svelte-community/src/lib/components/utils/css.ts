...
export function getResponsiveValue<T = string>(
	...
	if (typeof responsiveProp === "string") {
		return {
			[`--__${TOKEN_PREFIX}c-${componentName}-${componentProp}-xs`]: responsiveProp,
		};
	}
...
export function getResponsiveProps<T extends string>(
	...
	if (typeof responsiveProp === "string") {
		return {
			[`--__${TOKEN_PREFIX}c-${componentName}-${componentProp}-xs`]: translateTokenStringToCSS(
				componentProp,
				responsiveProp,
				tokenSubgroup,
				tokenExceptions,
	...
	Object.entries(responsiveProp).forEach(([breakpointAlias, aliasOrScale]) => {
		styleProps[`--__${TOKEN_PREFIX}c-${componentName}-${componentProp}-${breakpointAlias}`] =
