...
function readFailuresFromJSON(jsonPath: string): VisualFailure[] {
	if (!existsSync(jsonPath)) {
		console.log(`ℹ️  No visual test failures file found at ${jsonPath}`);
		return [];
	}
	...
	try {
		...
		console.error(`❌ Failed to read failures JSON: ${error}`);
		return [];
	}
}
...
function generateMarkdownComment(failures: VisualFailure[]): string {
	...
	let comment = `## ⚠️ Visual Test Failures\n\n`;
...
async function postGitHubComment(markdown: string) {
	...
	if (!token || !prNumber || !repository) {
		console.log("\n📝 Local execution - would post this comment:");
...
async function main() {
	...
	console.log("🔍 Reading visual test failures from JSON...");
	const failures = readFailuresFromJSON(jsonPath);

	if (failures.length === 0) {
		console.log("✅ No visual test failures found!");
		return;
	}
